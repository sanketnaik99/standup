...
export default function RoutineListView() {
    ...
    const navTitle = userStats
        ? `Routines | Lvl ${userStats.level} ${getProgressBar(userStats.xp, getXpRequiredForNextLevel(userStats.level))} ${userStats.xp}/${getXpRequiredForNextLevel(userStats.level)} XP 🔥 ${userStats.currentStreak}`
        : `Daily Routines`;
...
function RoutineItem({
    ...
    async function handleToggleStatus() {
        ...
        if (routine.status === "done" && newStatus !== "done") {
            await logRoutineCompletion(new Date(), false);
        }
    ...
    async function handleSetStatus(status: TaskStatus) {
        ...
        if (routine.status === "done" && status !== "done") {
            await logRoutineCompletion(new Date(), false);
        }
                        ...
                        <List.Item.Detail.Metadata>
                            ...
                            {routine.expectedDuration && (
                                <List.Item.Detail.Metadata.Label
                                    ...
                                    text={formatDuration(routine.expectedDuration)}
                                />
                            )}
                        </List.Item.Detail.Metadata>
                    }
...
function RoutineDetail({
    ...
    async function handleSetStatus(status: TaskStatus) {
        if (status === "done" && routine.status !== "done") {
            ...
            if (xpAwarded) {
                ...
                // onStatsChange is not passed to RoutineDetail, so we might miss updating the stats in the nav title immediately
                // but since we are in a stack, it might refresh when we pop?
                // Actually RoutineDetail doesn't have onStatsChange prop.
                await logRoutineCompletion(new Date(), true);
                return;
            }
        }
    ...
    async function handleToggleStatus() {
        ...
        if (newStatus === "done" && routine.status !== "done") {
            // Let handleSetStatus handle logic?
            // But handleSetStatus takes status argument.
            // We can just call handleSetStatus(newStatus)
